...
-- Atualizar função create_platform_checkout para aceitar código de referência
...
BEGIN
  ...
  IF referral_code_param IS NOT NULL AND referral_code_param != '' THEN
    ...
    RAISE NOTICE 'Código de referência % armazenado para processamento', referral_code_param;
  END IF;
...
-- Criar função para processar referrals de compras confirmadas
...
BEGIN
  ...
  IF webhook_event.provider = 'hotmart' THEN
    ...
    user_email := webhook_event.raw_payload->>'email' OR webhook_event.raw_payload->'buyer'->>'email';
  ...
  ELSIF webhook_event.provider = 'kiwify' THEN
    ...
    user_email := webhook_event.raw_payload->'Customer'->>'email' OR webhook_event.raw_payload->>'customer_email';
    purchase_amount := COALESCE(
  ...
  -- Se não conseguiu extrair email, usar dados básicos
  ...
  RAISE NOTICE 'Processando webhook: email=%, valor=%, plano=%', user_email, purchase_amount, plan_purchased;
  ...
  WHERE user_email = user_email
    AND NOT processed
    AND created_at > now() - interval '24 hours' -- Só nos últimos 24h
  ORDER BY created_at DESC
  LIMIT 1;
  
  IF referral_record IS NOT NULL THEN
    RAISE NOTICE 'Referral encontrado: código=%, valor=%', referral_record.referral_code, referral_record.purchase_amount;
  ...
  ELSE
    RAISE NOTICE 'Nenhum referral encontrado para %', user_email;
