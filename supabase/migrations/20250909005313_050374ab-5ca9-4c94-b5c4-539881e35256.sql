-- 1) Drop função existente se houver
DROP FUNCTION IF EXISTS public.normalize_webhook_payload(text, jsonb);

-- 2) Recriar função de normalização com processamento correto para Kiwify
...
-- 3) Ajustes na tabela de endpoints para controlar verificação
...
-- 4) Índice de idempotência para evitar duplicidade (se ainda não existir)
CREATE UNIQUE INDEX IF NOT EXISTS idx_webhook_events_idempotency_key ON public.webhook_events (idempotency_key);

-- 5) Realtime no perfil + replica identity para detectar diffs
DO $$ BEGIN
  BEGIN
