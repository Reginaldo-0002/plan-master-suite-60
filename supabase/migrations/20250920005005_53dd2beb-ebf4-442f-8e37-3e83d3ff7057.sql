-- Create normalize_webhook_payload to fix webhook processing
-- This function normalizes disparate provider payloads into a common schema
-- ensuring process_webhook_event can resolve user and plan

-- 1) Text-based entrypoint
...
BEGIN
  -- Common email fallbacks across providers
  v_email := COALESCE(
  ...
  IF v_provider = 'kiwify' THEN
    -- Kiwify: attempt to get product_id, subscription, amounts and dates
    ...
    -- Convert to cents (supports either cents already or BRL as decimal)
...
-- 2) Overload for platform_enum (if provider columns use this enum)
DO $$
BEGIN
  -- Create overload only if the type platform_enum exists
  IF EXISTS (
    SELECT 1 FROM pg_type WHERE typname = 'platform_enum'
  ) THEN
    EXECUTE $$
