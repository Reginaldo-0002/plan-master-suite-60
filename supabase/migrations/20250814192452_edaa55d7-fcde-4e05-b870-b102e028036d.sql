-- Fix Security Definer View issue by removing security_barrier and using proper RLS policies

-- Drop the existing view with security_barrier
DROP VIEW IF EXISTS public.user_accessible_resources;

-- Create a standard view without security_barrier property
...
-- Create RLS policy for the view that ensures users can only see their authorized resources
CREATE POLICY "Users can only view their accessible resources" ON public.user_accessible_resources
