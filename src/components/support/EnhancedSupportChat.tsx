...
export const EnhancedSupportChat = () => {
  ...
  useEffect(() => {
    if (currentView === 'chat' && !chatInitialized && !isChatBlocked) {
      initializeChat();
      setChatInitialized(true);
    }
  }, [currentView, chatInitialized, isChatBlocked]);
  ...
  const checkChatRestrictions = async () => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;

      // Check global chat restrictions
      ...
      if (userRestrictions && userRestrictions.length > 0) {
        const restriction = userRestrictions[0];
        if (restriction.blocked_until && new Date(restriction.blocked_until) > new Date()) {
          setIsChatBlocked(true);
          return;
        }
      }
  ...
  const sendMessage = async () => {
    if (!inputMessage.trim() || isChatBlocked) return;
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        // Create a temporary ticket for chat messages or use existing logic
        console.log('Saving chat message:', userMessage.content);
      }
    } catch (error) {
    ...
    <div className="fixed bottom-4 right-4 z-50">
      <Card className="w-96 h-[600px] shadow-xl">
        ...
        <CardContent className="p-4 h-[520px] flex flex-col">
          {isChatBlocked && currentView === 'chat' && (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center p-4">
                <MessageCircle className="w-12 h-12 mx-auto mb-4 text-muted-foreground" />
                <h3 className="font-medium mb-2">Chat Temporariamente Bloqueado</h3>
                <p className="text-sm text-muted-foreground mb-4">
                  O chat está temporariamente indisponível. Você pode criar um ticket de suporte para entrar em contato conosco.
                </p>
                <Button
          ...
          {!isChatBlocked && currentView === 'chat' && (
            <>
              ...
              <div className="flex gap-2">
                ...
                />
                <Button onClick={sendMessage} disabled={isChatBlocked}>
